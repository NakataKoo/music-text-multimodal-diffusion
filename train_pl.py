...
class MusicCaps(Dataset):
        ...
        def __getitem__(self, idx):
            ...
            if self.x == "audio" and self.c == "text":
                mel_latent = self.model.audioldm_encode(waveform[0].to(self.device)).detach() # transform mel-spectrogram（Tensor） into latent space
                text_emb = self.model.clip_encode_text([caption]).detach()
                return mel_latent, text_emb # data, condition
                #return waveform[0], caption
                ...
                return caption, waveform[0]

class LitModel(pl.LightningModule):
    def __init__(self, x, c, gpus):
        super().__init__()
        self.x = x
        self.c = c
        self.gpus = gpus
        self.model = self.model_define(self.x, self.c)

    def model_define(self, x, c):
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss = self.model.forward(x=data, c=condition, xtype=os.environ["XTYPE"], ctype='prompt')
    ...
    def configure_optimizers(self):
        ...
        optimizer = get_optimizer()(self.model, optimizer_config)
        self.ema = LitEma(self.model)
        return optimizer
...
def main():
    x = os.environ['XTYPE']
    c = os.environ['CTYPE']
    gpus = int(os.environ['NUM_GPUS_PER_NODE'])

    model = LitModel(x, c, gpus)
    trainer = pl.Trainer(
        max_epochs=2,
        gpus=gpus,
        ...
        callbacks=[ModelCheckpoint(dirpath='./model_checkpoints/', save_top_k=3, monitor='train_loss')],
        logger=True
    )
    trainer.fit(model)
